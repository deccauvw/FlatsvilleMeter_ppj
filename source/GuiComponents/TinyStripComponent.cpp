...
//            return;
    ...
    {
        std::string valueFormattedCh0 = std::format("{:.2f}", values.at(0));
        std::string valueFormattedCh1 = std::format("{:.2f}", values.at(1));
        auto s = juce::String(valueFormattedCh0) + juce::String("\t\t") + juce::String(valueFormattedCh1);
    ...
    {
        auto inputPeakValues = valueStringSupplierFn(); //vector
        for(int ch = 0;ch<inputPeakValues.size(); ch++)
        {
            inputPeakValues.at(ch) = juce::Decibels::gainToDecibels(inputPeakValues.at(ch));
        }
        setStringContent(inputPeakValues);
        repaint();
    }
